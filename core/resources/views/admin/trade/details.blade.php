...
@section('panel')
    <div class="row gy-4">
        ...
        <div class="col-lg-5">
            @if ($tradeDetails->status == 8 || $tradeDetails->status == 9)
            ...
            <div class="custom--card border">
                ...
                <div class="card-body">
                    <ul class="caption-list">
                        ...
                        @if ($tradeDetails->reported_by && $tradeDetails->status != 8)
                            <li>
                                <span class="caption">@lang('Reported By')</span>
                                <span class="value">
                                    @if ($tradeDetails->reported_by == $tradeDetails->seller_id)
                                        @lang('Seller')
                                    @else
                                        @lang('Buyer')
                                    @endif
                    ...
                    @if ($tradeDetails->status == 8)
                        <div class="d-flex flex-wrap gap-2 style--two text-center mt-4 border-top pt-3">
                            <button class="btn btn-md btn--danger confirmationBtn flex-fill" type="button" data-question="@lang('Are you sure to return') {{ $tradeDetails->crypto->code }} @lang('to seller') ?" data-action="{{ route('admin.trade.return', $tradeDetails->id) }}"> <i class="las la-undo"></i> @lang('In Favor of Seller')</button>

                            <button class="btn btn-md btn--success confirmationBtn flex-fill" type="button" data-question="@lang('Are you sure to release') {{ $tradeDetails->crypto->code }}?" data-action="{{ route('admin.trade.release', $tradeDetails->id) }}"> <i class="la la-check-circle" aria-hidden="true"></i> @lang('In Favor of Buyer')</button>
                        </div>
                    @endif

                    <div class="d-flex flex-wrap gap-3 mt-3 pt-3 border-top justify-content-center">
                        <a href="#0" class="text--primary terms">
                            <i class="la la-info-circle"></i> @lang('Terms of trade')
                        </a>

                        <a href="#0" class="text--primary payment">
