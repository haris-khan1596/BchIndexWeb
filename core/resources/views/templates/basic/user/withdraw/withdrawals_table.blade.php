                ...
                <tbody>
                    @foreach ($withdrawals as $withdrawal)
                        <tr>
                            ...
                            <td>
                                <strong>{{ showAmount($withdrawal->payable/$withdrawal->crypto->rate, 6) }} {{ __($withdrawal->crypto->code) }}</strong>
                            </td>

                            <td class="text-danger">
                                {{ showAmount($withdrawal->charge, 6) }} {{ __($withdrawal->crypto->code) }}
                            </td>

                            <td>
                                {{ showAmount($withdrawal->amount/$withdrawal->crypto->rate, 6) }} {{ __($withdrawal->crypto->code) }}
                            </td>
