...
class TradeController extends Controller
{
    ...
    public function details($uid)
    {
        ...
        if ($trade->status == 9 || $trade->status == 1 || $trade->status == 8) {
            $sellerEndTime          = null;
            $sellerRemainingMinutes = null;
            $sellerCancelMsz        = null;
            $sellerDisputeMsz       = null;
        ...
        if (($trade->status == 0) || ($trade->status == 2) || ($trade->status == 8)) {
            $chatPermission = 1;
        }

        $reviewIsPermitted = 0;

        if ($trade->status == 1 && $trade->reviewed == 0 && $trade->advertisement->user_id != auth()->id()) {
            $reviewIsPermitted = 1;
        }
    ...
    public function store(Request $request, $id)
    {
        ...
        $this->finalAmount = $request->amount;
