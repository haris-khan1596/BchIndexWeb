...
class TradeController extends Controller
{
    ...
    public function sendTradeRequest(Request $request, $id)
    {
        ...
        $finalAmount = $finalAmount + $tradeCharge;
        
        //convert 
        $blncmm = $finalAmount*$ad->crypto->rate;
        
        $wallet->balance -= $blncmm;//($finalAmount + $tradeCharge);
        $wallet->save();

        $transaction                     = new Transaction();
        $transaction->user_id            = $seller->id;
        $transaction->crypto_currency_id = $ad->crypto_currency_id;
        $transaction->amount             = $blncmm;
        ...
        $nnt = "<b>Notice to buyer</b><br>Please read the seller's terms and conditions carefully before clicking the paid icon and transfer the amount to the seller's account through your banking application. The seller's bank account details can be found in the payment details by going to the up order details. Fraud reporting: If the buyer asks to release the first crypto without paying, please immediately report to BCH INDEX Limited at this email (p2p@bchindex.com).";

        $chat           = new Chat();
        $chat->trade_id = $trade->id;
        $chat->user_id  = $user->id;
        $chat->message  = $nnt.'<br>'.$request->details;
        $chat->file     = null;
        $chat->file     = null;
        $chat->save();
    ...
    public function ajax_chat($id){
        ...
        return view($this->activeTemplate . 'user.trade.partials.chat_box_thread', compact('trade','profileImage'));
    }
    
    public function get_ajax_actions($id){
        $trade = Trade::where('uid', $id)->where(function ($q) {
            $q->orWhere('buyer_id', auth()->id())->orWhere('seller_id', auth()->id());
        })->with('chats')->firstOrFail();
        ...
        return view($this->activeTemplate . 'user.trade.partials.actions', compact('trade','profileImage'));
    }
    ...
    public function cancel($id)
    {
        ...
        $finalAmount = $trade->crypto_amount + $trade->trade_charge;
        
        //convert 
        $blncmm = $finalAmount*$trade->crypto->rate;
        ...
        $wallet->balance += $blncmm;//($trade->crypto_amount + $trade->trade_charge);
        $wallet->save();

        $transaction                        = new Transaction();
        $transaction->user_id               = $trade->seller->id;
        $transaction->crypto_currency_id    = $trade->crypto_currency_id;
        $transaction->amount                = $trade->amount;//$trade->crypto_amount;
        $transaction->post_balance          = $wallet->balance;
    ...
    public function release($id)
    {
        ...
        $wallet->balance += $blncmm;//$trade->crypto_amount;
        $wallet->save();

        $transaction               = new Transaction();
        $transaction->user_id      = $trade->buyer->id;
        $transaction->crypto_currency_id    = $trade->crypto_currency_id;
        $transaction->amount       = $blncmm;//$trade->crypto_amount;
        $transaction->post_balance = $wallet->balance;
        $transaction->charge       = 0;
        $transaction->trx_type     = '+';
