...
class AdvertisementController extends Controller
{
    ...
    public function store(Request $request, $id = 0)
    {
        ...
        if ($check[0] == 'success') {
            ...
            $user           = auth()->user();
            $createdAd      = Advertisement::where('user_id', $user->id)->where('status',1)->where('type',$request->type)->where('crypto_currency_id', $request->crypto_id)->count();
            
             if($createdAd > 0) {
                 
                 $notify[] = ['error', 'You have already active advertisement in same type & crypto currency!'];
                 
                 return back()->withNotify($notify);
             }
        }
        
        $userWallet = Wallet::where('crypto_currency_id', $request->crypto_id)->first();
