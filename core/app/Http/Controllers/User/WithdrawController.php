...
class WithdrawController extends Controller
{
    ...
    public function withdrawMoney($code)
    {
        ...
        $withdrawals = Withdrawal::where('user_id', auth()->id())->where('status', '!=', 0)->where('crypto_currency_id', $crypto->id)->with('crypto')->paginate(getPaginate());
    ...
    public function store(Request $request)
    {
        ...
        $transaction->details            = showAmount($withdraw->payable/$withdraw->crypto->rate, 6) . ' ' . $withdraw->crypto->code . ' Withdraw Successful ';
        ...
        notify($user, 'WITHDRAW_REQUEST', [
            'amount'       => showAmount($withdraw->amount/$withdraw->crypto->rate, 6),
            'payable'      => showAmount($withdraw->payable/$withdraw->crypto->rate, 6),
            'charge'       => showAmount($withdraw->charge/$withdraw->crypto->rate, 6),
            'currency'     => $withdraw->crypto->code,
            'trx'          => $withdraw->trx,
            'post_balance' => showAmount($userWallet->balance/$userWallet->crypto->rate, 6)
        ]);
    ...
    public function log(Request $request)
    {
        ...
        $withdrawals = Withdrawal::where('user_id', auth()->id())->where('status', '!=', 0);
