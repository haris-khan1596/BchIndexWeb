                    ...
                    <div class="profile-sidebar">
                        ...
                        <div class="profile-sidebar__widget">
                            ...
                            <ul class="profile-verify-list">
                                <li class="@if ($user->ev) verified @else unverified @endif"><i class="las la-envelope"></i>
                                    @if ($user->ev)
                                        @lang('Email Verified')
                                    @else
                                ...
                                <li class="@if ($user->sv) verified @else unverified @endif"><i class="las la-mobile-alt"></i>
                                    @if ($user->sv)
                                        @lang('SMS Verified')
                                    @else
                                        @lang('SMS Unverified')
                                ...
                                <li class="@if ($user->kv == 1) verified @else unverified @endif"><i class="las la-user-check"></i>
                                    @if ($user->kv == 1)
                                        @lang('KYC Verified')
                                    @else
                                        @lang('KYC Unverified')
                    ...
                    <div class="row gy-4">
                        ...
                        @foreach ($wallets as $wallet)
                            <div class="col-xl-4 col-md-6 d-widget-item">
                                <a class="d-block" href="{{ route('user.transaction.index') }}?crypto={{ $wallet->crypto->id }}">
                                    <div class="d-widget">
                                        <div class="d-widget__icon">
                                            <img src="{{ getImage($walletImage->path . '/' . $wallet->crypto->image, $walletImage->size) }}" alt="">
                                        </div>
